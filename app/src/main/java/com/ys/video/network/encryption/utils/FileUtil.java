...
import com.ys.video.network.encryption.encryption.MDUtil;
...
public class FileUtil {
     ...
     * 压缩并保存图片至指定大小
     *
     * @param sizeType {@link FileSizeUtil#SIZE_TYPE_MB} {@link FileSizeUtil#SIZE_TYPE_KB}
     * @param size 文件压缩到指定大小
     * @return 保存文件路径
     */
    public static File saveBitmapWithLimit(Context context, String filePath, String fileName, Bitmap bitmap, int sizeType, double size) {
        String path = saveFile(context, filePath, fileName, bitmap);
        if (!TextUtils.isEmpty(path)) {
            File file = new File(path);
            if (file.exists()) {
                if (sizeType != 0 && FileSizeUtil.getFileOrFilesSize(path, FileSizeUtil.SIZE_TYPE_MB) >= size) {
                    Bitmap compress = BitmapUtil.compressImage(bitmap, 80);
                    saveBitmapWithLimit(context, filePath, fileName, bitmap, sizeType, size);
                }
                return file;
            }
        }
            ...
            if (file.exists()) {
                if (sizeType != 0) {
                    if (FileSizeUtil.getFileOrFilesSize(path, FileSizeUtil.SIZE_TYPE_MB) >= size) {
                        ToastUtil.toast("保存图片到本地,并返回File");
                        return null;
                    }
                }
                return file;
